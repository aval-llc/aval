...
test("GPT-6 Luna tool calls use the Chat Completions compatible reasoning mode", async () => {
  ...
  try {
    const response = await callOpenAiCompatible(
      { baseUrl: "https://api.openai.com/v1", apiKey: "test-only", model: "gpt-6-luna", providerLabel: "OpenAI" },
      {
