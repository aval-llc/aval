...
import { maintenanceContext, createInboundWorkOrder } from './maintenance-intake';
const text = { type:'string' };
export const COMMUNICATION_TOOLS: ToolSchema[] = [
 { name: 'read_maintenance_context', description: 'Match this inbound Gmail message to one workspace resident and active lease. An unresolved match requires human review.', input_schema: { type: 'object', properties: { conversation_id: text, message_id: text }, required: ['conversation_id', 'message_id'] } },
 { name: 'create_maintenance_work_order', description: 'Propose an internal maintenance work order for this exact matched message. Requires human approval; never dispatches vendors or spends money.', input_schema: { type: 'object', properties: { conversation_id: text, message_id: text, resident_id: text, property_id: text, unit_id: text, summary: { type: 'string', minLength: 1, maxLength: 2000 }, priority: { type: 'string', enum: ['routine', 'urgent', 'emergency'] } }, required: ['conversation_id','message_id','resident_id','property_id','unit_id','summary','priority'] } },
...
export async function runCommunicationTool(dbSession: DbSession, name:string,args:Record<string,unknown>,org:string,key?:string) {
  if (name === 'read_maintenance_context') return maintenanceContext(dbSession, org, String(args.conversation_id), String(args.message_id));
  if (name === 'create_maintenance_work_order') return createInboundWorkOrder(dbSession, org, args, key ?? '');
  ...
  if (name === 'send_external_message') {
    if (typeof args.conversation_id === 'string') return replyToConversation(dbSession, org,args.conversation_id,String(args.body),key,typeof args.message_id === 'string' ? args.message_id : undefined);
