...
function StandardConnectionDialog({ provider, onClose, onRefresh }: { provider: Provider | null; onClose: () => void; onRefresh: () => void }) {
  ...
  if (!provider) return null; const providerGuide = guides[provider.id] ?? { aval: ["Secure provider adapter"], customer: ["Administrator consent"], proof: "The upstream identity is verified before sync." }; const blocker = provider.setupBlocker ?? connectionBlocker(provider.id); const blocked = Boolean(blocker) || provider.configured === false;
  const connect = async (event?: FormEvent) => {
    ...
    try {
      ...
      setMessage(t(provider.category === "Model" ? "ConnectionDialog.connectionVerified" : "ConnectionDialog.identityVerifiedInitialSyncIsQueued"));
