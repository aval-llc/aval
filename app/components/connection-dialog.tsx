...
export type Provider = {
  ...
  defaultModel?: string;
  setupBlocker?: string;
  readiness?: { sync: boolean; verification: boolean };
...
const guides: Record<string, { aval: string[]; customer: string[]; proof: string }> = {
  ...
  anthropic: { aval: ["Anthropic Messages API adapter", "Encrypted key storage", "Same guarded agent loop as every model provider"], customer: ["An Anthropic account", "An API key with billing enabled", "Choose the exact model in Advanced"], proof: "Aval lists your account's available models before marking this connected." },
...
export function ConnectionDialog({ provider, onClose, onRefresh }: { provider: Provider | null; onClose: () => void; onRefresh: () => void }) {
  ...
  if (!provider) return null; const providerGuide = provider.id === "sap_bydesign" ? { aval: [t("ConnectionDialog.sap.adapter"), t("ConnectionDialog.sap.storage")], customer: [t("ConnectionDialog.sap.admin"), t("ConnectionDialog.sap.fields")], proof: t("ConnectionDialog.sap.proof") } : guides[provider.id] ?? { aval: ["Secure provider adapter"], customer: ["Administrator consent"], proof: "The upstream identity is verified before sync." }; const blocker = provider.setupBlocker ?? connectionBlocker(provider.id); const blocked = Boolean(blocker) || provider.configured === false;
  const connect = async (event?: FormEvent) => {
    event?.preventDefault();
    if (blocked || status === "working") return;
    setStatus("working"); setMessage("");
    try {
      const response = await fetch("/api/integrations/connect", {
        method: "POST", headers: { "content-type": "application/json" },
        body: JSON.stringify({ provider: provider.id, credentials: provider.authMode === "oauth2" ? undefined : credentials, returnTo: `${window.location.pathname}?view=connections` }),
      });
      const data = await response.json() as { authorizationUrl?: string; error?: string; required?: string[]; missing?: string[]; connection?: { id: string } };
      if (!response.ok) throw new Error(`${data.error ?? t("ConnectionDialog.connectionFailed")}${data.missing ? `: ${data.missing.join(", ")}` : ""}`);
      if (data.authorizationUrl) { window.location.assign(data.authorizationUrl); return; }
      if (!data.connection?.id) throw new Error(t("ConnectionDialog.connectionFailed"));
      ...
      const verified = await verification.json() as { error?: string; connection?: { externalAccountName?: string } };
      if (!verification.ok) {
        setStatus("error");
        setMessage(provider.id === "sap_bydesign" ? t("ConnectionDialog.sap.failed") : `${t("ConnectionDialog.credentialsEncryptedProviderActionRemains")} ${verified.error ?? provider.note}`);
        notify(t("ConnectionDialog.setupSaved"), provider.title); onRefresh(); return;
      }
      setCredentials({}); setStatus("saved");
      setMessage(t(provider.id === "sap_bydesign" ? "ConnectionDialog.sap.verified" : provider.category === "Model" ? "ConnectionDialog.connectionVerified" : "ConnectionDialog.identityVerifiedInitialSyncIsQueued"));
      celebrate(t("ConnectionDialog.connectionVerified"), verified.connection?.externalAccountName ?? provider.title);
      onRefresh();
    } catch (error) { setStatus("error"); setMessage(error instanceof Error ? error.message : t("ConnectionDialog.connectionFailed")); }
  };
  const validate = async () => {
    if (!provider.connection?.id || status === "working") return;
    setStatus("working"); setMessage("");
    try {
      const response = await fetch("/api/integrations/validate", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ connectionId: provider.connection.id }), signal: AbortSignal.timeout(60000) });
      const result = await response.json() as { error?: string };
      if (!response.ok) throw new Error(result.error ?? t("ConnectionDialog.connectionFailed"));
      setStatus("idle"); setMessage(t("ConnectionDialog.accessValidated")); onRefresh();
    } catch (error) { setStatus("error"); setMessage(error instanceof Error ? error.message : t("ConnectionDialog.connectionFailed")); }
  };
  return <Dialog.Root open onOpenChange={(open) => !open && onClose()}><Dialog.Portal><Dialog.Overlay className="dialog-overlay"/><Dialog.Content className="connection-dialog"><div className="dialog-top"><BrandMark provider={provider.id}/><Dialog.Close className="icon-button" aria-label={t("ConnectionDialog.cancel")}><Xmark width={20} height={20}/></Dialog.Close></div><Dialog.Title>{provider.title}</Dialog.Title><Dialog.Description>{provider.id === "sap_bydesign" ? t("ConnectionDialog.sap.description") : provider.description}</Dialog.Description><div className="dialog-status-row"><span><ShieldCheck width={16} height={16}/>{providerIsReadOnly(provider.id, provider.readOnly) ? t("ConnectionDialog.readAccess") : t("ConnectionDialog.twoWayChannel")}</span><span><Key width={16} height={16}/>{provider.id === "sap_bydesign" ? t("ConnectionDialog.sap.credentials") : provider.authMode.replace("_", " ")}</span><span><NetworkLeft width={16} height={16}/>{provider.webhook ? t("ConnectionDialog.webhookEvents") : t("ConnectionDialog.accessVerification")}</span></div><Tabs.Root defaultValue="readiness"><Tabs.List className="dialog-tabs three"><Tabs.Trigger value="readiness">{t("ConnectionDialog.readiness")}</Tabs.Trigger><Tabs.Trigger value="access">{t("ConnectionDialog.access")}</Tabs.Trigger><Tabs.Trigger value="flow">{t("ConnectionDialog.flow")}</Tabs.Trigger></Tabs.List><Tabs.Content value="readiness"><div className="readiness-grid"><div><p>{t("ConnectionDialog.avalConfiguresOnce")}</p>{providerGuide.aval.map((item) => <span key={item}><Check width={15} height={15}/>{item}</span>)}</div><div><p>{t("ConnectionDialog.customerBrings")}</p>{providerGuide.customer.map((item) => <span key={item}><User width={15} height={15}/>{item}</span>)}</div></div><p className="proof-note"><ShieldCheck width={16} height={16}/>{providerGuide.proof}</p>{blocked && <div className="setup-warning"><strong>{t("ConnectionDialog.notYetAvailableTitle")}</strong><p>{blocker ? t("ConnectionDialog.adapterUnavailableBody") : t("ConnectionDialog.notYetAvailableBody")}</p><Foldout summary={t("ConnectionDialog.technicalReferenceForSupport")}><span>{blocker ?? provider.env.join(" · ")}</span></Foldout></div>}</Tabs.Content><Tabs.Content value="access"><div className="permission-box"><p>{t("ConnectionDialog.avalWillRequest")}</p>{provider.permissions.map((permission) => <span key={permission}><Check width={16} height={16}/>{provider.id === "sap_bydesign" ? t("ConnectionDialog.sap.permission") : permission}</span>)}</div><p className="connection-note">{provider.id === "sap_bydesign" ? t("ConnectionDialog.sap.note") : provider.note}</p></Tabs.Content><Tabs.Content value="flow"><ol className="architecture-list"><li><b>01</b><span><strong>{t("ConnectionDialog.authorize")}</strong>{t("ConnectionDialog.consentIsScopedToThisWorkspace")}</span></li><li><b>02</b><span><strong>{t("ConnectionDialog.verify")}</strong>{provider.id === "sap_bydesign" ? t("ConnectionDialog.sap.adapter") : t("ConnectionDialog.avalTestsTheActualUpstreamIdentity")}</span></li><li><b>03</b><span><strong>{provider.id === "sap_bydesign" ? t("ConnectionDialog.sap.next") : t("ConnectionDialog.normalize")}</strong>{provider.id === "sap_bydesign" ? t("ConnectionDialog.sap.mapping") : t("ConnectionDialog.sourceIdsAndTimestampsRemainTraceable")}</span></li></ol></Tabs.Content></Tabs.Root>{!blocked && provider.authMode === "qr_link" && <div className="qr-link-panel"><QrPlaceholder/><div><p>{t("ConnectionDialog.qrLinkInstructions")}</p><span className="qr-link-status"><span className="presence-dot"/>{status === "working" ? t("ConnectionDialog.waitingForScan") : t("ConnectionDialog.qrLinkReady")}</span></div></div>}{provider.id === "sap_bydesign" && <p className="connection-note">{t("ConnectionDialog.sap.note")}</p>}{!blocked && provider.authMode !== "oauth2" && provider.authMode !== "qr_link" && <form className="credential-form" onSubmit={connect}>{provider.credentialFields?.map((field) => <label key={field.key}>{provider.id === "sap_bydesign" ? t(`ConnectionDialog.sap.labels.${field.key}`) : field.label}<input type={field.secret ? "password" : "text"} value={credentials[field.key] ?? ""} onChange={(event) => setCredentials((current) => ({ ...current, [field.key]: event.target.value }))} autoComplete="off" required/></label>)}{provider.category === "Model" && <Foldout summary={t("ConnectionDialog.advanced")}><label>{t("ConnectionDialog.modelOverrideLabel")}<select className="model-select" value={credentials.model ?? ""} onChange={(event) => setCredentials((current) => ({ ...current, model: event.target.value }))}><option value="">{t("ConnectionDialog.modelRecommended", { model: provider.defaultModel ?? "" })}</option>{modelOptionsFor(provider.id).map((id) => <option key={id} value={id}>{id}</option>)}</select></label>{supportsReasoningEffort(credentials.model || provider.defaultModel || "") && <label>{t("ConnectionDialog.reasoningEffortLabel")}<select className="model-select" value={credentials.reasoningEffort ?? ""} onChange={(event) => setCredentials((current) => ({ ...current, reasoningEffort: event.target.value }))}><option value="">{t("ConnectionDialog.reasoningEffortDefault")}</option>{REASONING_EFFORT_LEVELS.map((level) => <option key={level} value={level}>{level}</option>)}</select></label>}<p className="foldout-hint">{t("ConnectionDialog.modelOverrideHint")}</p></Foldout>}</form>}{provider.id === "quickbooks" && provider.connection?.status === "connected" && <IntegrationSyncControls provider={provider.id}/>}{message && <p role="status" className={`dialog-message ${status}`}>{message}</p>}<div className="dialog-actions">{provider.connection?.status === "connected" && provider.category !== "Model" && !blocked && <button className="soft-button" disabled={status === "working"} onClick={() => void validate()}>{t("ConnectionDialog.validateAccess")}</button>}<button className="soft-button" onClick={onClose}>{t("ConnectionDialog.cancel")}</button><button className="primary-button" disabled={status === "working" || status === "saved" || blocked} onClick={() => connect()}>{status === "working" ? (provider.authMode === "qr_link" ? t("ConnectionDialog.waitingForScan") : t("ConnectionDialog.verifying")) : status === "saved" ? t("ConnectionDialog.saved") : blocked ? t("ConnectionDialog.avalSetupRequired") : provider.authMode === "oauth2" ? t("ConnectionDialog.continueToAuthorization") : provider.authMode === "qr_link" ? t("ConnectionDialog.linkDevice") : provider.id === "sap_bydesign" ? t("ConnectionDialog.sap.test") : t("ConnectionDialog.encryptVerify")}<NavArrowRight width={17} height={17}/></button></div></Dialog.Content></Dialog.Portal></Dialog.Root>;
}
